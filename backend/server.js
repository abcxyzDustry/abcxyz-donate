...
async function initializeDatabase() {
  try {
    ...
    await pool.query('DROP TABLE IF EXISTS shopbank_accounts CASCADE');
    ...
    // Create ShopBank accounts table
    await pool.query(`
      CREATE TABLE IF NOT EXISTS shopbank_accounts (
        id SERIAL PRIMARY KEY,
        username VARCHAR(255) UNIQUE NOT NULL,
...
// Sync ShopBank accounts from plugin
app.post('/api/shopbank/sync', async (req, res) => {
  try {
    const { accounts, server_name, api_key } = req.body;
    ...
    if (api_key !== 'shopbank_sync_key_2024') {
      return res.status(401).json({ error: 'Invalid API key' });
    }

    let syncedCount = 0;
    for (const account of accounts) {
      if (!account.username || account.balance === undefined) continue;

      await pool.query(`
        INSERT INTO shopbank_accounts (username, player_name, balance, server_name, last_sync)
      ...
      `, [account.username, account.player_name || account.username, account.balance, server_name || 'unknown']);
    ...
    console.error('ShopBank sync error:', error);
    res.status(500).json({ error: 'Sync failed: ' + error.message });
  }
});

// Get top 30 richest accounts
app.get('/api/shopbank/leaderboard', async (req, res) => {
  try {
    const limit = 30;
    
    // Get total stats
    const statsResult = await pool.query(`
      SELECT COUNT(*) as total_accounts, COALESCE(SUM(balance), 0) as total_credits
      FROM shopbank_accounts
    `);
    
    // Get top 30 accounts by balance
    const result = await pool.query(`
      SELECT username, player_name, balance, server_name, last_sync
      FROM shopbank_accounts
    ...
    res.json({
      ...
      accounts: result.rows.map((row, idx) => ({
        rank: idx + 1,
...
// Get all accounts for plugin (export)
app.get('/api/shopbank/export/:api_key', async (req, res) => {
  try {
    ...
    const result = await pool.query(`
      SELECT username, player_name, balance, server_name
      FROM shopbank_accounts
      ORDER BY balance DESC
    `);
